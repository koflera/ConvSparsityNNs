...
class Dyn2DRadEncObj(nn.Module):
	...
	def apply_A(self, x_tensor):
		...
		mb = x_tensor.shape[0] 
		Nt = self.im_size[2]
		kdat_tensor = torch.zeros(mb,self.Nc,2,self.Nrad, Nt).to(x_tensor.device)
		for kt in range(Nt):
			kdat_tensor[...,kt] = self.NUFFT(x_tensor[...,kt],self.ktraj_tensor[...,kt])
	...
	def apply_AH(self, k_tensor):
		...
		mb = k_tensor.shape[0] 
		Nt = self.im_size[2]
		xshape = (mb,1,2,) + tuple(self.im_size)
		xrec_tensor = torch.zeros( xshape).to(k_tensor.device)
		for kt in range(Nt):
			xrec_tensor[...,kt] = self.AdjNUFFT(k_tensor[...,kt],self.ktraj_tensor[...,kt])
	...
	def apply_W12(self, k_tensor, dcomp_tensor):
		
		return torch.sqrt(dcomp_tensor)*k_tensor
	
	def apply_W12A(self, x):
		
		return torch.sqrt(self.dcomp_tensor)*self.apply_A(x)
	
	def apply_AHW12(self, k_tensor):
		
		return self.apply_AH( torch.sqrt(self.dcomp_tensor)*k_tensor)
	...
	def apply_AHA_Toeplitz(self, x):
		...
		x_toep_tensor = torch.zeros( x.shape).to(x.device)
		Nt = self.im_size[2]
		for kt in range(Nt):
			x_toep_tensor[...,kt] = self.ToepNUFFT(x[...,kt],self.AHA_toep_kernel_list[kt])
	...
	def apply_AdagA_Toeplitz(self, x):
		...
		Nt = self.im_size[2]
		for kt in range(Nt):
			x_toep_tensor[...,kt] = self.ToepNUFFT(x[...,kt],self.AdagA_toep_kernel_list[kt])
